...
function booksReducers() {
  ...
  switch (action.type) {
    ...
    case 'DELETE_BOOK':
      currentBook = [].concat(_toConsumableArray(state.books));
      // Determine at which index in booksarray is the book to be deleted
      index = currentBook.findIndex(function (book) {
        return book.id === action.payload.id;
      });

      return { books: [].concat(_toConsumableArray(currentBook.slice(0, index)), _toConsumableArray(currentBook.slice(index + 1))) };
    case 'UPDATE_BOOK':
      currentBook = [].concat(_toConsumableArray(state.books));
      index = currentBook.findIndex(function (book) {
        return book.id === action.payload.id;
      });
      newBook = _extends({}, currentBook[index], { title: action.payload.title });
      return { books: [].concat(_toConsumableArray(currentBook.slice(0, index)), [newBook], _toConsumableArray(currentBook.slice(index + 1))) };
    default:
      return state;
  }
}
